...
const handleReaction = (reaction, user, adding) => {
  ...
  if (
    ...
    guild.me.Permission.has(Permissions.MANAGE_ROLES)
  ) {
...
module.exports = async (client) => {
  ...
  for (const result of results) {
    ...
    if (!guild) {
      console.log(`Noņem guild ID "${guildId}" no datubāzes`)
    ...
    if (!channel) {
      console.log(`Noņem channel ID "${channelId}" no datubāzes`)
    ...
    try {
      ...
      console.log(`Noņem message ID "${messageId}" no datubāzes`)
