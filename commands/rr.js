...
module.exports = {
  minArgs: 3,
  expectedArgs: '<Emoji> <Role nosaukums, tags, vai ID> <Role nosaukums>',
  requiredPermission: ['ADMINISTRATOR'],
  callback: async ({ message, args }) => {
    const { guild } = message

    if (!guild.me.Permission.has(Permissions.MANAGE_ROLES)) {
    ...
    if (!newRole) {
      message.reply(`Nevarēja atrast role "${role}"`)
      return
    }
    ...
    if (!fetchedMessage) {
      message.reply('Kaut kāds errors, lūdzu mēģini vēlreiz')
      return
    }
