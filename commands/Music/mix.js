	...
	run: async (client, message, args) => {
		try {
			...
			let newMsg = await message.reply({
				content: `${client.allEmojis.loading} Lādē **'${args[0] ? args[0] : "Kaut kādu"}' mūzikas mix**`,
			});
			try {
				...
				//Edit the reply
				newMsg.edit({
					content: `${queue?.songs?.length > 0 ? "👍 Ielādēts" : "🎶 Pašlaik atskaņo"}: **'${args[0] ? args[0] : "Kaut kāds"}'**`,
				});
			} catch (e) {
